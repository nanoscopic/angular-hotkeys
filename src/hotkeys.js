// Original Copyright (c) 2016 Wes Cruver
// Copyright (c) 2020 David Helkowski
// License: MIT

angular.module('cfp.hotkeys', []).provider('hotkeys', ['$injector', 'angularMousetrapProvider'], function( $injector, MousetrapProvider ) {
  var Mousetrap = MousetrapProvider.$get();
  ...
  this.Mousetrap = function ($rootElement, $rootScope, $compile, $window, $document) {
    ...
    function _del (hotkey) {
      ...
      if (angular.isArray(combo)) {
        var retStatus = true;
        var i = combo.length;
        while (i--) retStatus = _del(combo[i]) && retStatus;
        return retStatus;
      }
      else {
    ...
    function _get (combo) {
      ...
      for (var i = 0; i < scope.hotkeys.length; i++) {
        ...
        if (hotkey.combo.indexOf(combo) > -1) return hotkey;
      }
    ...
    function bindTo (scope) {
      ...
      if (!(scope.$id in boundScopes)) {
        ...
        scope.$on('$destroy', function () {
          ...
          while (i--) _del(boundScopes[scope.$id].pop());
        });
      }
